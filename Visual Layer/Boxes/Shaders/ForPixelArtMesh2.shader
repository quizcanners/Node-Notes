		...
		SubShader{
			Pass{
				...
				struct v2f {
					...
					float4 hold : TEXCOORD6;
					float2 perfuv : TEXCOORD7;
					SHADOW_COORDS(8)
				...
				float4 frag(v2f i) : COLOR{
					...
					float3 nn2 = UnpackNormal(tex2D(_BumpDetail, i.texcoord.xy*(4 + nn.xy)));
					nn += nn2 * (1 - c.a) * smudge *0.05;
					...
					float4 light = (tex2Dlod(_MainTex, float4(pixuv + nn.xy*_MainTex_TexelSize.xy*(deBord), 0, 0)));
					...
					light.rgb *= (_LightColor0.rgb * (1 - diff) + ambientCol.rgb) * deBord;

					float deUp = (1 - nn.z);


					col.rgb = (
						col.rgb * (light.rgb  + _LightColor0 * diff + ambientCol)
					...
					col.rgb += pow(NdotH, 128 * gloss) * gloss * gloss * _LightColor0 * diff * (1.1- gloss) * 0.5 ;
